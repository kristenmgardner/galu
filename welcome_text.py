...
def get_export_job(everyaction_download_url, everyaction_headers, everyaction_auth):
    ...
    while time.time() < timeout_start + timeout:
        ...
        jobstatus = response.json().get('jobStatus')
...
def prepare_data(downloadLink):
    ...
    if len(df) > 0:
        ...
        logger.info("No new contacts. Exiting.")
    ...
    if len(df_filtered) > 0:
        ...
        logger.info("No contacts that were created today. Exiting.")
    ...
    if len(df_for_strive) != 0:
        ...
        logger.info("No opted in contacts. No contacts to send to Strive. Exiting.")
...
if __name__ == "__main__":
	...
	if downloadLink is not None:
        	...
        	logger.info("No data found. No new contacts to add.")
